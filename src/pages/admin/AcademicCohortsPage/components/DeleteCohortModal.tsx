...
export function DeleteCohortModal({ open, onClose, onConfirm }: DeleteCohortModalProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Xác nhận xóa khóa học"
    >
      <button
        type="button"
      ...
      <div className="relative z-10 w-full max-w-md rounded-xl border border-sand-200 bg-white p-4 shadow-lg animate-in fade-in zoom-in-95 duration-200">
        <h3 className="text-base font-semibold text-sand-900">Xác nhận xóa khóa học</h3>
        <p className="mt-1 text-sm font-medium text-sand-600">
          Bạn có chắc chắn muốn xóa khóa học này khỏi hệ thống? Thao tác này sẽ không thể hoàn tác và chỉ có thể thực hiện nếu khóa học chưa có sinh viên nào đăng ký.
        </p>
