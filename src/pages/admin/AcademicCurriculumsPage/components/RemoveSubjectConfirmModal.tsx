...
interface RemoveSubjectConfirmModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: () => void;
  subjectName: string;
...
export function RemoveSubjectConfirmModal({
  open,
  onClose,
  onConfirm,
  subjectName,
  curriculumName,
}: RemoveSubjectConfirmModalProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Xác nhận gỡ môn học"
    >
      <button
        type="button"
      ...
      <div className="relative z-10 w-full max-w-md rounded-xl border border-sand-200 bg-white p-4 shadow-lg animate-in fade-in zoom-in-95 duration-200">
        <h3 className="text-base font-semibold text-sand-900">Xác nhận gỡ môn học</h3>
        <p className="mt-1 text-sm font-medium text-sand-600">
          Bạn có chắc chắn muốn gỡ môn học <span className="font-semibold text-sand-900">"{subjectName}"</span> ra khỏi chương trình đào tạo <span className="font-semibold text-sand-900">"{curriculumName}"</span> không?
        </p>
