...
export function DeleteCurriculumModal({ open, onClose, onConfirm }: DeleteCurriculumModalProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Xác nhận xóa chương trình đào tạo"
    >
      <button
        type="button"
      ...
      <div className="relative z-10 w-full max-w-md rounded-xl border border-sand-200 bg-white p-4 shadow-lg animate-in fade-in zoom-in-95 duration-200">
        <h3 className="text-base font-semibold text-sand-900">Xác nhận xóa chương trình</h3>
        <p className="mt-1 text-sm font-medium text-sand-600">
          Bạn có chắc chắn muốn xóa chương trình đào tạo này khỏi hệ thống? Thao tác này sẽ gỡ bỏ toàn bộ lộ trình môn học liên quan và không thể khôi phục lại.
        </p>
